...
from test_common import single_node_tree, sample_data
...
def population_with_trees(sample_data, sample_trees):
    ...
    X, y = sample_data
    pop = Population(X, y, population_size=len(sample_trees), 
                    attributes=[0], possible_thresholds={0: [1.0]}, 
                    max_depth=1, p_split=0.5)
