...
def simple_population(sample_data):
    ...
    return Population(X, y, population_size=1, attributes=attributes,
                     possible_thresholds=possible_thresholds, max_depth=3, p_split=0.5)
...
def test_mutate_leaf_node(simple_population, single_node_tree):
    original_tree = DecisionTree(
        attributes=single_node_tree.attributes.copy(),
        thresholds=single_node_tree.thresholds.copy()
    )
    ...
    assert single_node_tree.attributes[0] == None
